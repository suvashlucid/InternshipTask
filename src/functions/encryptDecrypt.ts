...
const encrypt = (plainText: any) => {
  try {
    ...
    const asciiValueToString = asciiValue.map((value) =>
      String.fromCharCode(value)
    );
    const plainString = asciiValueToString.join("");
    const replaceString = [...plainString]
      .map((char) => keyCollection[char] || char)
      .join("");
...
const decrypt = (encrypted: any) => {
  try {
    ...
    const plainString = [...replaceString]
      .map((char) => keyCollection[char] || char)
      .join("");
    const asciiValueWithLength = [...plainString].map((char) =>
      char.charCodeAt(0)
    );
    const string = asciiValueWithLength
